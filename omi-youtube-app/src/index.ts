...
server.tool(
  ...
  async ({ action }) => {
    try {
      ...
      const jsCommand = jsCommands[action as keyof typeof jsCommands];
...
server.tool(
  ...
  async ({ query, maxResults = 3 }) => {
    try {
      ...
      const resultsText = searchResults.map((video: any, index: number) =>
...
server.tool(
  ...
  async ({ videoId, keyword }) => {
    try {
      ...
      const resultsText = matches.slice(0, 5).map(match =>
        `At ${match.timestamp}: "${match.text}"`
      ).join('\n\n');
...
async function searchYouTubeWithYtdlp(query: string, maxResults: number): Promise<Array<any>> {
  return new Promise((resolve) => {
    const ytDlp = spawn('yt-dlp', [
...
async function downloadVtt(url: string): Promise<string | null> {
  return new Promise((resolve) => {
...
function findKeywordMatches(vttContent: string, keyword: string) {
  const matches: Array<{ timestamp: string; text: string }> = [];
